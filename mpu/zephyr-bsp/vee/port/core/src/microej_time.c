...
int64_t microej_time_get_time_nanos(void) {
	...
	if ((system_time_ns == 0) ||
	    (k_uptime_get() - (system_time_ns / 1000000)) >
	    ((uint64_t)(UINT32_MAX) * 1000) / CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC) {
		/* if the time is not initialized yet or if the 32 bits high-performance counter has overflowed between two
		...
		system_time_ns += ((int64_t)(k_cycle_get_32() - system_hp_cycle) * 1000000000) /
